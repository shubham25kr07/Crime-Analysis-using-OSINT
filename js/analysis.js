...
// Function to make AJAX request everytime class .form-control is altered
	$('.form-control').change(function() {
		...
		function graph5(data){
				...
				//console.log(offence1);
				for(var i =0; i < data.length; i++){
					if(data[i].properties.primary_type == offence1){
						arr1.push(data[i].properties.location_description);
					}
				}
				// console.log(arr1.length);

				//console.log(offence2);
				for(var i =0; i < data.length; i++){
					if(data[i].properties.primary_type == offence2){
						arr2.push(data[i].properties.location_description);
					}
				}
				// console.log(arr2.length);
			
				var result1 = foo(arr1);
				var result2 = foo(arr2);
				...
				for(var i=0; i<result1[0].length; i++)
					processed1.push([result1[0][i],result1[1][i]]);

				processed1.sort(function(a, b) {
			    	return b[1] - a[1];
				});
				for(var i=0; i<processed1.length; i++){
					result1[0][i]= processed1[i][0];
					result1[1][i]= processed1[i][1];
				}

				for(var i=0; i<result2[0].length; i++){
					var f=0;
					for(var j=0;j<10;j++){
						if(result1[0][j] == result2[0][i]){
							f= 1;
							break;
						}
					}
					if(f)
						processed2.push([result2[0][i],result2[1][i]]);
				}
				for(var i=0; i<processed2.length; i++){
					result2[0][i]= processed2[i][0];
					result2[1][i]= processed2[i][1];
				}

				var trace1 =
						  {
						x: result1[0].slice(1,10),
						y: result1[1].slice(1,10),
					    type: "bar",
					    textfont: {
					        size: 12,
					        color: '#000'
					...
					var trace2 =
						  {
						x: result2[0].slice(1,12),
						y: result2[1].slice(1,12),
					    type: "bar",
					    textfont: {
					        size: 12,
					        color: '#000'
				...
				var layout = {
					  title: 'Crime by area of occurence',
					  // xaxis: {
					  // 	size: 12;
					  // }
					  yaxis: {
			...
			// PLOT crime by time- graph 6

			var formLineGraph61 = new FormData();
		    var formLineGraph61 =  {
			...
			var formLineGraph62 = new FormData();
		    var formLineGraph62 =  {
		        	"$select" : "date as date1"
		        	+ ", "
		        	+ "date_trunc_ym(date) as date2"
			...
			$.ajax({
				...
				for(var i =0; i < data1.length; i++){
					// if(i%200 == 0)
					// 	console.log(data[i].properties.primary_type)
					// if(data[i].properties.primary_type == offence1){
						var hour= new Date(data1[i].properties.date1).getHours();
						arr1.push(hour/3);
					// }
				}
				for(var i =0; i < data2.length; i++){
					// console.log(data[i].properties.primary_type)
					// if(data[i].properties.primary_type == offence2){
						var hour= new Date(data2[i].properties.date1).getHours();
						arr2.push(hour/3);
						// console.log(hour/3);
					// }
				}
				...
				var lineGraph= [scatter1, scatter2];
				var layout= {
